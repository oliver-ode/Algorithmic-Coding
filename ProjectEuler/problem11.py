...
for y in range(20):
    for x in range(20-3):
        if a[y][x]*a[y][x+1]*a[y][x+2]*a[y][x+3] > maxProd:
            maxProd = a[y][x]*a[y][x+1]*a[y][x+2]*a[y][x+3]
# Vertical
for x in range(20):
    for y in range(20-3):
        if a[y][x]*a[y+1][x]*a[y+2][x]*a[y+3][x] > maxProd:
            maxProd = a[y][x]*a[y+1][x]*a[y+2][x]*a[y+3][x]
# Down right
for y in range(20-3):
    for x in range(20-3):
        if a[y][x]*a[y+1][x+1]*a[y+2][x+2]*a[y+3][x+3] > maxProd:
            maxProd = a[y][x]*a[y+1][x+1]*a[y+2][x+2]*a[y+3][x+3]
# Down Left
for y in range(20-3):
    for x in range(19, 0+2, -1):
        if a[y][x]*a[y+1][x-1]*a[y+2][x-2]*a[y+3][x-3] > maxProd:
            maxProd = a[y][x]*a[y+1][x-1]*a[y+2][x-2]*a[y+3][x-3]
print(maxProd)